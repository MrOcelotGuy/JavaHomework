...
class AVLTree2<E extends Comparable<E>> extends BST2<E> {
	...
	private void balancePath(E e) {
		...
		for (int i = path.size() - 1; i >= 0; i--) {
			...
			updateHeight(A);
			updateSize(A); // update size
			AVLTree2Node<E> parentOfA = (A == root) ? null : (AVLTree2Node<E>) (path.get(i - 1));
	...
	private int balanceFactor(AVLTree2Node<E> node) {
		...
		} else if (node.left == null) // node has no left subtree
		{
			...
			return ((AVLTree2Node<E>) node.right).height - ((AVLTree2Node<E>) node.left).height;
		}
	}
	 ...
	 */
	protected static class AVLTree2Node<E extends Comparable<E>> extends BST2.TreeNode<E> {

		protected int height = 0; 
		protected int size = 0; // New data field
